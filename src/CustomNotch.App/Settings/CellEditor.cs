...
/// <summary>Une cellule en quatre cartes : Source (le formulaire de son schéma), Affichage, Actions, Groupe. Chaque
...
public sealed class CellEditor : UserControl
{
    ...
    public CellEditor(SettingsContext ctx, CellConfig cell, Action<Action> run)
    {
        ...
        body.Children.Add(ActionsSection());
        body.Children.Add(GroupSection());
        Content = body;
    }
    ...
    {
        if (_cell.IsGroup)
            return Bricks.Card(this, "Source", Ui.Text("Un groupe n'a pas de source : son anneau et sa carte viennent de ses enfants.", 12, null, "Muted"));
        ...
        children.Add(Bricks.Action(this, "Changer de source", () =>
        {
        ...
        return Bricks.Card(this, "Source", children.ToArray());
    }
    ...
    private UIElement DisplaySection()
    {
        var children = new List<UIElement>
        {
            Bricks.Row("Libellé", Bricks.Debounced(_cell.Label ?? "", v => Set(c => { if (v.Length == 0) c.Remove("label"); else c["label"] = v; }), name: "label")),
            Bricks.Row("Glyph", GlyphGallery.Build(this, _cell.Glyph, g => Set(c => { if (g is null) c.Remove("glyph"); else c["glyph"] = g; }))),
        };

        if (!_cell.IsGroup)
        {
            children.Add(Bricks.Combo(this, "Type de rendu", Kinds, _cell.Kind ?? "", v => Set(c => { if (v.Length == 0) c.Remove("kind"); else c["kind"] = v; })));

            var refresh = new StackPanel { Orientation = Orientation.Horizontal };
            var known = Refreshes.Any(r => r.Item1 == (_cell.Refresh ?? ""));
            var combo = ComboBare(Refreshes, known ? _cell.Refresh ?? "" : "", v => Set(c => { if (v.Length == 0) c.Remove("refresh"); else c["refresh"] = v; }));
            var custom = Bricks.Field(this, known ? "" : _cell.Refresh ?? "", v => Set(c => { if (v.Length == 0) c.Remove("refresh"); else c["refresh"] = v; }), 100, "refresh");
            ...
            children.Add(Bricks.Row("Cadence de lecture", refresh));
        }
        ...
        // gestionnaire passé à Bricks.Field() dans leur propre initialisation — sans ça, le compilateur voit une
        // variable qui se référence elle-même avant d'être assignée (CS0165).
        TextBox warn = null!, crit = null!;
        CheckBox invert = null!;
        warn = Bricks.Field(this, _cell.Thresholds?.Warn?.ToString(CultureInfo.InvariantCulture) ?? "", _ => CommitThresholds(), 90, "warn");
        crit = Bricks.Field(this, _cell.Thresholds?.Crit?.ToString(CultureInfo.InvariantCulture) ?? "", _ => CommitThresholds(), 90, "crit");
        ...
        children.Add(Bricks.Row("Seuils (% ou valeur)", thresholds));

        children.Add(Bricks.Combo(this, "Légende sous la cellule", Captions, _cell.Caption switch { null => "", true => "on", false => "off" }, v => Set(c => { if (v.Length == 0) c.Remove("caption"); else c["caption"] = v == "on"; })));
        children.Add(Bricks.Combo(this, "Activité", Activities, _cell.Activity ?? "", v => Set(c => { if (v.Length == 0) c.Remove("activity"); else c["activity"] = v; })));

        return Bricks.Card(this, "Affichage", children.ToArray());
    ...
    private UIElement ActionsSection()
    {
        ...
        TextBox value = null!;
        ComboBox kind = null!;
        value = Bricks.Field(this, click?.Open ?? click?.Shell ?? click?.Source ?? "", _ => CommitClick(), 320, "click");
        kind = ComboBare(ClickKinds, current, _ => CommitClick());
        var panel = new StackPanel { Orientation = Orientation.Horizontal };
        value.Margin = new Thickness(8, 0, 0, 0);
        panel.Children.Add(kind); panel.Children.Add(value);
        stack.Children.Add(Bricks.Row("Au clic", panel));
        ...
        void Render()
        {
            ...
            for (var i = 0; i < list.Count; i++)
            {
                ...
                // Label et valeur : commit à l'Entrée / perte de focus (Bricks.Field), pas à chaque touche —
                // reconstruire la ligne à chaque frappe démonterait le TextBox en pleine saisie. Seul ce qui change
                // la forme des lignes (genre, ordre, ajout/suppression) reconstruit (Render()).
                row.Children.Add(Bricks.Field(this, a.Label ?? "", v => { a.Label = v; CommitCard(rebuild: false); }, 140, $"act_label_{index}"));
                var k = ComboBare(ActionKinds, a.Open is not null ? "open" : a.Shell is not null ? "shell" : "source", v => { var val = a.Open ?? a.Shell ?? a.Source ?? ""; a.Open = a.Shell = a.Source = null; if (v == "open") a.Open = val; else if (v == "shell") a.Shell = val; else a.Source = val; CommitCard(); });
                k.Margin = new Thickness(8, 0, 0, 0); row.Children.Add(k);
                var v = Bricks.Field(this, a.Open ?? a.Shell ?? a.Source ?? "", t => { if (a.Open is not null) a.Open = t; else if (a.Shell is not null) a.Shell = t; else a.Source = t; CommitCard(rebuild: false); }, 220, $"act_value_{index}");
                v.Margin = new Thickness(8, 0, 0, 0); row.Children.Add(v);
                row.Children.Add(Bricks.Btn(this, "↑", () => { if (index > 0) { (list[index - 1], list[index]) = (list[index], list[index - 1]); CommitCard(); } }, "GhostButton"));
                row.Children.Add(Bricks.Btn(this, "↓", () => { if (index < list.Count - 1) { (list[index + 1], list[index]) = (list[index], list[index + 1]); CommitCard(); } }, "GhostButton"));
                row.Children.Add(Bricks.Btn(this, "✕", () => { list.RemoveAt(index); CommitCard(); }, "GhostButton"));
                buttons.Children.Add(row);
            }
            buttons.Children.Add(Bricks.Btn(this, "+ Bouton", () => { list.Add(new ActionConfig { Label = "Ouvrir", Open = "https://" }); CommitCard(); }));
        }
        Render();
        stack.Children.Add(Bricks.Row("Boutons de la carte", buttons));
        return Bricks.Card(this, "Actions", stack);
    ...
    private UIElement GroupSection()
    {
        ...
        if (candidates.Count == 0) { stack.Children.Add(Muted("Aucune autre cellule dans cette pilule.")); return Bricks.Card(this, "Groupe", stack); }
        ...
        stack.Children.Add(Bricks.Row("Enfants", boxes));
        var heads = new[] { ("", "Le pire enfant") }.Concat(children.Select(id => (id, _ctx.Store.Current.Cell(id)?.Label ?? id))).ToArray();
        stack.Children.Add(Bricks.Combo(this, "Tête (anneau de la pilule)", heads, _cell.Headline ?? "", v => Set(c => { if (v.Length == 0) c.Remove("headline"); else c["headline"] = v; })));
        stack.Children.Add(Muted("Cocher au moins un enfant fait de cette cellule un groupe : sa source n'est alors plus lue."));
        return Bricks.Card(this, "Groupe", stack);
    ...
    /// <summary>Une liste déroulante nue (sans ligne de formulaire), pour composer une ligne à plusieurs contrôles
    /// (cadence de lecture, au clic, genre d'une action de carte) — Bricks.Combo rend toute la ligne, trop pour ces
    /// cas-là.</summary>
    private ComboBox ComboBare(IReadOnlyList<(string Value, string Text)> options, string current, Action<string> commit)
    {
        var combo = new ComboBox { HorizontalAlignment = HorizontalAlignment.Left };
